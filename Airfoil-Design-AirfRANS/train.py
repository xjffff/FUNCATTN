...
def main(device, train_dataset, val_dataset, Net, hparams, path, criterion='MSE', reg=1, val_iter=10,
         name_mod='GraphSAGE', val_sample=True, wandb_cfg=None):
    '''
        Args:
    ...
    wb = None
    if wandb_cfg and wandb_cfg.get("enable", False) and wandb is not None:
        wb_init_kwargs = {
    ...
    for epoch in pbar_train:
        ...
        if wb is not None:
            log_dict = {
                ...
                "train/loss_surf_mean": float(loss_surf),
                "train/loss_vol_mean": float(loss_vol),
                "train/surf_vx": float(loss_surf_var[0]),
                "train/surf_vy": float(loss_surf_var[1]),
                "train/surf_p": float(loss_surf_var[2]),
                "train/surf_nut": float(loss_surf_var[3]),
                "train/vol_vx": float(loss_vol_var[0]),
                "train/vol_vy": float(loss_vol_var[1]),
                "train/vol_p": float(loss_vol_var[2]),
                "train/vol_nut": float(loss_vol_var[3]),
            }
            try:
        ...
        if val_iter is not None:
            if epoch % val_iter == val_iter - 1 or epoch == 0:
                ...
                if wb is not None:
                    wandb.log({
                        "val/loss_surf_mean": float(val_surf),
                        "val/loss_vol_mean": float(val_vol),
                        "val/surf_vx": float(val_surf_var[0]),
                        "val/surf_vy": float(val_surf_var[1]),
                        "val/surf_p": float(val_surf_var[2]),
                        "val/surf_nut": float(val_surf_var[3]),
                        "val/vol_vx": float(val_vol_var[0]),
                        "val/vol_vy": float(val_vol_var[1]),
                        "val/vol_p": float(val_vol_var[2]),
                        "val/vol_nut": float(val_vol_var[3]),
                    }, step=epoch)
            else:
                pbar_train.set_postfix(train_loss=train_loss, loss_surf=loss_surf, val_loss=val_loss, val_surf=val_surf)
        else:
    ...
    if wb is not None:
        try:
            wandb.summary["nb_parameters"] = float(params_model)
        except Exception:
            pass
    sns.set()
    ...
    if wb is not None:
        try:
            art = wandb.Artifact(f"{name_mod}-model", type="model")
            art.add_file(osp.join(path, 'model'))
