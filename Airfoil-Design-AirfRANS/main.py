...
parser.add_argument('--wandb_project', type=str, default=None,
                    help='W&B project name. Leave None to disable wandb.')
...
for i in range(args.nmodel):
    ...
    if args.model == 'Transolver':
        ...
        model = Transolver(n_hidden=256,
                           ...
                           slice_num=32,
                           unified_pos=1).cuda()
    elif args.model == 'TransFmap':
        from models.FuncAttn import FuncAttn

        model = FuncAttn(n_hidden=256,
                           n_layers=8,
                           space_dim=7,
                           fun_dim=0,
                           n_head=8,
    ...
    wandb_cfg = {
        "enable": args.wandb_project is not None and args.wandb_mode != "disabled",
    ...
    model = train.main(device, train_dataset, val_dataset, model, hparams, log_path,
                       criterion='MSE_weighted', val_iter=10, reg=args.weight, name_mod=args.model, val_sample=True, wandb_cfg=wandb_cfg)
    print('end training')
    models.append(model)
