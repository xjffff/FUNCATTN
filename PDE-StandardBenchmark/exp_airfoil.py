...
parser.add_argument('--save_name', type=str, default='airfoil_FuncAttn')
...
def main():
    ...
    pos = torch.stack([inputX, inputY], dim=-1)

    output = np.load(OUTPUT_Sigma)[:, 4]
    output = torch.tensor(output, dtype=torch.float)
    print(pos.shape, output.shape)

    pos_train = pos[:ntrain, ::r1, ::r2][:, :s1, :s2]
    y_train = output[:ntrain, ::r1, ::r2][:, :s1, :s2]
    pos_test = pos[ntrain:ntrain + ntest, ::r1, ::r2][:, :s1, :s2]
    y_test = output[ntrain:ntrain + ntest, ::r1, ::r2][:, :s1, :s2]

    pos_train = pos_train.reshape(ntrain, -1, 2)
    pos_test = pos_test.reshape(ntest, -1, 2)
    ...
    train_loader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(pos_train, pos_train, y_train),
        batch_size=args.batch_size,
        shuffle=True,
        generator=g
    )
    test_loader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(pos_test, pos_test, y_test),
        batch_size=args.batch_size,
        shuffle=False
    )
        ...
        with torch.no_grad():
            for pos, fx, y in test_loader:
                ...
                if id < showcase:
                    ...
                    plt.axis('off')
                    plt.pcolormesh(
                        x[0, :, 0].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        x[0, :, 1].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        np.zeros([140, 35]),
                        shading='auto',
                        edgecolors='black',
                        linewidths=0.1
                    ...
                    plt.pcolormesh(
                        x[0, :, 0].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        x[0, :, 1].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        out[0, :].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        shading='auto',
                        cmap='coolwarm'
                    )
                    plt.colorbar()
                    ...
                    plt.pcolormesh(
                        x[0, :, 0].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        x[0, :, 1].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        y[0, :].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        shading='auto',
                        cmap='coolwarm'
                    )
                    plt.colorbar()
                    ...
                    plt.pcolormesh(
                        x[0, :, 0].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        x[0, :, 1].reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        (out[0, :] - y[0, :]).reshape(s1, s2)[40:180, :35].detach().cpu().numpy(),
                        shading='auto',
                        cmap='coolwarm'
                    )
                    plt.colorbar()
