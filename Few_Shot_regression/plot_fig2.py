...
from data import SineCfg, SineTask
...
def mse_vs_K(model, K_list, *, episodes=1000, M=200, cfg=SineCfg(), device='cpu'):
    """Return dict mapping K -> mean MSE."""
    results = {}
    for K in K_list:
...
def plot_fig2(curves, save_path='figure2.pdf'):
    """Plot MSE vs K for each (label, mse_dict) pair in *curves*."""
...
if __name__ == '__main__':
    ...
    MODELS = {
        'Attention': (
            lambda: Attention(d_model=256, num_heads=4),
            'checkpoints/attention_k32.pth',
        ),
        'Intention': (
            lambda: Intention(latent_dim=1000, ridge=1e-4),
            'checkpoints/intention_k32.pth',
        ),
        'Transolver': (
            lambda: Transolver(latent_dim=256, num_groups=8, init_param=False),
            'checkpoints/transolver_k32.pth',
        ),
        'Ours': (
            lambda: FuncAttn(latent_dim=128, num_groups=8, ridge=1e-4, init_param=True),
            'checkpoints/funcattn_k32.pth',
        ),
    }
    ...
    plot_fig2(curves, save_path='figure2.pdf')
